(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(hl-line ((t (:background "dark green" :foreground "gray92"))))
 '(org-date ((t (:foreground "dim gray" :underline t))))
 '(org-special-keyword ((t (:inherit font-lock-keyword-face :foreground "dim gray"))))
 '(outline-3 ((t (:foreground "dark green"))))
 '(outline-4 ((t (:foreground "black")))))

(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(custom-safe-themes
   '("00b4f46a5aff4a9a3e38a249a622153f9ef9f9a990c4409a3da2d1eb5de1401a"
     "c5e7a36784b1955b28a89a39fef7c65ddc455b8e7fd70c6f5635cb21e4615670"
     default))
 '(frame-inhibit-implied-resize '(tab-bar-lines tool-bar-lines menu-bar-lines))
 '(package-selected-packages
   '(chruby company consult consult-projectile embark embark-consult
	    expand-region goto-chg gptel groovy-mode js2-mode
	    js2-refactor json-mode markdown-mode multiple-cursors
	    nimbus-theme orderless org-roam projectile
	    projectile-rails projectile-ripgrep shx smart-jump
	    smart-region smartparens transpose-frame vertico web-mode
	    xref-js2 yaml-mode yasnippet)))
